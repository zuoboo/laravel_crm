...
<template>
        ...
        <div class="modal__overlay" tabindex="-1" data-micromodal-close>
            <div class="modal__container w-2/3" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
                            ...
                            <tbody>
                                <tr v-for="customer in customers.value.data" :key="customer.id">
                                    <td class="border-b-2 border-gray-200 px-4 py-3">
                                        <button @click="setCustomer({ id: customer.id, kana: customer.kana })" type="button" class="text-blue-400">{{ customer.id }}</button></td>
                ...
                <footer class="modal__footer">
                    <button @click="toggleStatus" class="modal__btn" data-micromodal-close
    ...
    <button class="flex mx-auto text-white bg-teal-500 border-0 mt-2 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg" @click="searchCustomers" data-micromodal-trigger="modal-1">検索する</button>
</template>
